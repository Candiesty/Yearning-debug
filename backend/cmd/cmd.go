...
var RunServer = &gcli.Command{
	...
	Func: func(c *gcli.Command, args []string) error {
		model.DBNew(RunOpts.config)
		service.UpdateData()
		service.StartYearning(net.JoinHostPort(RunOpts.addr, RunOpts.port), false)
		return nil
	},
}

var Debug = &gcli.Command{
	Name: "debug",
	Desc: "启动调试",
	Config: func(c *gcli.Command) {
		c.StrOpt(&RunOpts.addr, "addr", "a", "0.0.0.0", "Yearning启动地址")
	...
	Func: func(c *gcli.Command, args []string) error {
		model.DBNew(RunOpts.config)
		service.UpdateData()
		service.StartYearning(net.JoinHostPort(RunOpts.addr, RunOpts.port), true)
		return nil
	},
}
