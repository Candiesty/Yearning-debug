...
// 自定义连接数据库函数
func DBNew_other(p_dsn DSN) {
	p_dsn.DBName = "db1"
	dsn := fmt.Sprintf("host=%s user=%s password=%s dbname=%s port=%d sslmode=disable TimeZone=Asia/Shanghai", p_dsn.Host, p_dsn.Username, p_dsn.Password, p_dsn.DBName, p_dsn.Port)
	fmt.Print(dsn)
